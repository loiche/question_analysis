...
class StParametersCooccurenceNW:    
    ...
    # marker size
    center_nw_marker_size = 30
    connected_nw_marker_size = 10
    # nw layout
    nw_layout_width = 1200
    nw_layout_height = 600
    ...
    def set_coefficient_of_restitution(self, side=False):
        ...
        単語間の距離（デフォルト30）:数値が上げると接続している単語同士は離れ、逆に接続していない単語同士は近づくよう配置されます。
        """
        if side:
    ...
    def set_NW_marker_size(self):
        self.center_nw_marker_size = st.sidebar.number_input(label="ユーザーが選択した単語の円の大きさ（デフォルト:30）",
                                                        min_value=5,
                                                        max_value=300,
                                                        step=10,
                                                        value=30)
        self.connected_nw_marker_size = st.sidebar.number_input(label="ユーザーが選択した単語と接続する単語の円の大きさ（デフォルト:10）",
                                                        min_value=5,
                                                        max_value=300,
                                                        step=10,
                                                        value=10)

    def set_nw_layout_parameters(self):
        self.nw_layout_height = st.sidebar.number_input(label="図の縦幅（デフォルト：600）",
                                                    min_value=300,
                                                    max_value=3000,
                                                    step=100,
                                                    value=600)
        self.nw_layout_width = st.sidebar.number_input(label="図の横幅（デフォルト：1200）",
                                                min_value=300,
                                                max_value=3000,
                                                step=100,
                                                value=1200)
...
def return_G(jaccard_df, word_count_partofspeech_df, user_multiselected_word_list, StParameters, node_criterion_paircount): 
    ...
    # ユーザーによって選ばれた単語のみ残す
    first_match_bool_list = [True if ii in set(user_multiselected_word_list) else False for ii in jaccard_df["first_word_in_pair"].values]
    second_match_bool_list = [True if ii in set(user_multiselected_word_list) else False for ii in jaccard_df["second_word_in_pair"].values]
    tmp_jaccard_df = jaccard_df.loc[first_match_bool_list or second_match_bool_list, :]

    #グラフの作成
    word_connection_dict = {}
    for word_pair, row in tmp_jaccard_df.iterrows():
    ...
    #sizeの設定
    size_list = [StParameters.center_nw_marker_size if adjacencies[0] in user_multiselected_word_list else StParameters.connected_nw_marker_size for node, adjacencies in enumerate(graph.adjacency())]
...
def make_cooccurrence_nw_plotly(graph, color_list, size_list, StParameters):   
    ...
    fig.update_layout(
        width=StParameters.nw_layout_width,
        height=StParameters.nw_layout_height,
    )
...
class CooccurenceNWPage:
    def main(self):
        ...
        SPCNW.set_NW_marker_size()
        SPCNW.set_nw_layout_parameters()
        ...
        select_word_type = st.selectbox(label=f"初期単語群を選択してください。ここで選択された単語群は{page_number}.2.2の初期単語に反映されます。",
                                        options=["形容詞の中で出現頻度top10の単語", "全単語の中で出現頻度top15の単語", "名詞の中で出現頻度top10の単語"])
        if select_word_type == "全単語の中で出現頻度top10の単語":
        ...
        elif select_word_type == "形容詞の中で出現頻度top10の単語":
        ...
        if len(user_multiselected_word_list) > 0:
            with st.spinner():
                ...
                graph, color_list, size_list = return_G(jaccard_df=jaccard_df, 
                                                        ...
                                                        user_multiselected_word_list=user_multiselected_word_list,
                                                        StParameters=SPCNW,
        ...
        st.subheader(f"{page_number}.4 共起ネットワークの作図に用いる単語ペアの繋がりの強さの算出表")
        st.markdown("""
        共起ネットワークに用いる単語ペアの繋がりの強さの算出表です。
        単語ペアの出現数、単語ペア内の各単語の出現数を用いてJaccar係数を算出しています。
        共起ネットワークではユーザーが指定した単語に対してJaccard係数の高かった単語ペア上位5個を可視化します。
        """)
        #- 表頭の単語をクリックすると、列がその単語の出現数でソートされます。
        #- 表頭の単語にマウスを当てると、その単語の左に漢字の「三」のような3本の線が表示されます。その3本線をクリックすると、エクセル同様のフィルタを列に掛けることができます。
        sents_array = make_sents_array(questionnaire_raw_df)
        escape_user_multiselected_word_list = [_.replace("+", "\+") for _ in user_multiselected_word_list]
        word_feature_df = get_count_of_target_pattern(words_in_sent_df["words_in_sent"].values, 
                                            sents_array,
                                            escape_user_multiselected_word_list, #candidates_words_list,
                                            "count")
        ...
        word_feature_agg_df.columns = user_multiselected_word_list
        word_feature_agg_df = word_feature_agg_df.reset_index()
        #AgGrid(word_feature_agg_df)
        agg_jaccard_df = jaccard_df.reset_index()
        agg_jaccard_df.columns = ["単語ペア（[単語1, 単語2]）", "単語ペアの出現数", "単語1", "単語2", "単語1の出現数", "単語2の出現数", "Jaccard係数"]
        AgGrid(agg_jaccard_df)
        #st.markdown("""
        #**利用上のポイント**
