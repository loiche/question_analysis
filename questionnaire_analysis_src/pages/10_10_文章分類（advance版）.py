...
def main():
    ...
    if len(word_bag) >= cluster_num:
        ...
        #単語が一つもマッチしないものを分離
        #ilocにしている
        match_word_feature_df = word_feature_df[word_feature_df.iloc[:, 1:].sum(axis=1) >= 1]
        not_match_word_feature_df = word_feature_df[word_feature_df.iloc[:, 1:].sum(axis=1) < 1]
        ...
        km = KMeans(n_clusters=cluster_num,
                ...
                random_state=0)
        #ilocにしている
        y_km = km.fit_predict(match_word_feature_df.iloc[:, 1:])
